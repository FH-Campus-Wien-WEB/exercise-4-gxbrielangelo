    ...
    .then((results) => {
      ...
      for (const movie of results) {
        const container = new ElementBuilder("div")
          .with("id", movie.imdbID)
          .class("search-result");
        container.append(
          new ElementBuilder("h3").text(`${movie.Title} (${movie.Year || ""})`),
        );
        const addBtn = new ButtonBuilder("Add").onclick(() => {
          addMovie(movie.imdbID)
            .then((res) => {
              if (res && res.status === 201) {
                const el = document.getElementById(movie.imdbID);
                if (el) el.remove();
              }
            })
            .catch(() => {
              // addMovie already handles alerts/logging
            });
        });

        container.append(addBtn);
        container.appendTo(resultsDiv);
      }
    })
    .catch((error) => {
    ...
    if (currentSession) {
      ...
      if (loginTime) {
        try {
          ...
          text = `Hi ${firstName} ${lastName}, du hast dich am ${day}. ${month} ${year} um ${hours}:${minutes} angemeldet.`;
        } catch (err) {
